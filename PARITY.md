...
| Epic grouping (Neovim) | Same parent-based grouping, and bd-list-tree parity: a dependency targeting an in-result epic nests the dependent beneath it — under *every* epic it depends on, with `├──`/`└──` connectors and `│` continuation gutters, roots = never-a-child, order = (priority asc, natural id asc). Emacs keeps parent-only flat grouping (see §11). | `nvim/lua/chaplet/list.lua:M.group_by_epic` |
...
| Neovim | PARITY 6 — List view | Dependency-nesting parity: parentless beads (including epics) nest beneath every in-result epic they depend on, duplicating rows across dependency paths exactly like `bd list`'s tree; Emacs keeps parent-only grouping with one row per bead. | Roadmap databases express epic hierarchy through `blocks` dependencies (e.g. trawler-incremental); the Neovim list mirrors `bd list`'s recursive tree so both surfaces show identical structure. Rows are duplicated by design; status-bar counts and detail targets stay deduplicated by id. |
| Neovim | PARITY 6 — List view | Dependency cycles among epic targets render the cycle members flat after the tree instead of dropping them (`bd list` prints nothing). | Losing beads from the list repeats the "not seeing all beads" failure; flat fallback keeps every bead reachable. |
| Neovim | PARITY 6 — List view | `|` opens the bead detail in a new vertical split (graph: open focused node the same way); `q` in that split kills the pane. Same-window `<CR>` behavior and Emacs keymaps are unchanged. | Neovim-first split workflow for keeping the list visible beside detail; Emacs users retain single-window semantics. |
| Neovim | PARITY 3 — Read surface | Detail reads fetch comments only when the show payload reports `comment_count > 0` (`M.detail`); Emacs fetches comments on every detail read. | Embedded Dolt spins a whole engine per bd spawn and takes an exclusive lock (gastownhall/beads#5397), so every skipped spawn halves detail latency; comment-free beads are the common case. |
